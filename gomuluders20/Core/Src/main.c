...
int main(void)
{
  ...
  MX_TIM1_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_Encoder_Start(&htim1, TIM_CHANNEL_ALL); // hangi timera bağlı ,hangi channler aktif edilmeli
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  encValue = (TIM1 -> CNT ) >> 2; // timer1’deki counter değerini arttır demektir.
  }
  /* USER CODE END 3 */
}
